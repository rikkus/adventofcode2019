defmodule Aoc7 do
  ...
  def part_two(mem, opts \\ []) do
    permutations([5, 6, 7, 8, 9])
    |> Enum.map(fn permutation -> do_part_two(:array.from_list(mem), permutation, opts) end)
    |> Enum.max()
  end

  def do_part_two(mem, [p1, p2, p3, p4, p5], opts \\ []) do
    # IO.inspect([p1, p2, p3, p4, p5])
    c1 = Computer.new("1", mem, [p1, 0], opts)
    c2 = Computer.new("2", mem, [p2], opts)
    c3 = Computer.new("3", mem, [p3], opts)
    c4 = Computer.new("4", mem, [p4], opts)
    c5 = Computer.new("5", mem, [p5], opts)
  ...
  def do_part_two(c1, c2, c3, c4, c5, max \\ 0) do
    ...
    %Computer{state: state, outputs: [last_output | _]} = result
    = Computer.execute(c1, after_output: :idle)
