defmodule Computer do
  ...
  def do_execute(computer) do
    computer
    |> execute_instruction(
      Instruction.decode(computer.mem, computer.ip, debug: computer.debug)
    )
  end

  defp execute_instruction(%Computer{mem: mem, ip: ip, relative_base: relative_base, debug: debug} = computer, {:add, [a, b, x] = args}) do
  ...
  defp execute_instruction(%Computer{mem: mem, ip: ip, relative_base: relative_base, debug: debug} = computer, {:multiply, [a, b, x] = args}) do
  ...
  defp execute_instruction(%Computer{mem: mem, ip: ip, relative_base: relative_base, inputs: [x | xs], debug: debug} = computer, {:set, [index] = args}) do
    log("set #{inspect(index)} to #{inspect(x)}", computer)

    %Computer{computer | mem: Memory.write(mem, ip, relative_base, index, x, debug: debug), inputs: xs}
  ...
  defp execute_instruction(%Computer{mem: mem, ip: ip, relative_base: relative_base, outputs: outputs, after_output: :continue, debug: debug} = computer, {:get, [value] = args}) do
    log("get #{inspect(value)} (then continue)", computer)

    value_ = Memory.read(mem, ip, relative_base, value, debug: debug)

    %Computer{computer | outputs: [value_ | outputs]} |> move_to_next_instruction(args) |> do_execute()
  end

  defp execute_instruction(%Computer{mem: mem, ip: ip, relative_base: relative_base, outputs: outputs, after_output: :idle, debug: debug} = computer,
         {:get, [value] = args}
       ) do
    ...
    %Computer{computer | outputs: [value_ | outputs], state: :idle} |> move_to_next_instruction(args)
  end

  defp execute_instruction(%Computer{mem: mem, ip: ip, relative_base: relative_base, ic: ic, debug: debug} = computer, {:jump_if_one, [a, b]}) do
  ...
  defp execute_instruction(%Computer{mem: mem, ip: ip, relative_base: relative_base, ic: ic, debug: debug} = computer, {:jump_if_zero, [a, b]}) do
    log("jump to #{inspect(a)} if #{inspect(b)} == 0", computer)
  ...
  defp execute_instruction(%Computer{mem: mem, ip: ip, relative_base: relative_base, debug: debug} = computer, {:set_if_less, [a, b, c] = args}) do
  ...
  defp execute_instruction(%Computer{mem: mem, ip: ip, relative_base: relative_base, debug: debug} = computer, {:set_if_equal, [a, b, c] = args}) do
    log("set #{inspect(c)} to 1 if #{inspect(a)} == #{inspect(b)}, else 0", computer)
